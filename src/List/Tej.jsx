...
export default function Tej(props) {
    const {
        IoMdSend,
        MdKeyboardVoice,
        FaCamera,
    ...
    const locl = () => {
        ...
        const data = localStorage.getItem("whatsapptej")
        if (data) {
    ...
    useEffect(() => {
        localStorage.setItem("whatsapptej", JSON.stringify(Messagelist))
            ...
            <div className='containercc'>
                <div className='chatBox'>
                    ...
                    <div className='messageBox'>
                        <div className='message w-auto'>
                            WhatsApp clone projeni çok beğendim Enes. Tebrikler!<br />
                            <span className='time'>11:56</span>
                        </div>

                        {Messagelist.map((e, index) => (
                            <div key={index} className='message w-auto' style={{ textAlign: 'right' }}>
                                <p>
                                    {e.name} <br /> <span className='time'>{e.ate}</span>
                                </p>
                                <div>
                                    <img src={e.selectedImage} alt='user upload' style={{ maxWidth: '100px' }} /> <br />
                                    <span className='time'>{e.ate}</span>
                                </div>
